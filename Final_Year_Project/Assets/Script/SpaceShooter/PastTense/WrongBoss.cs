...
public class WrongBoss : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        transform.GetChild(shoulddestroyship).GetComponentInChildren<Canvas>().GetComponentInChildren<Text>().text = JSONReader.instance.GetPastSimpleWrong();
        transform.GetChild(shoulddestroyship).GetComponent<WrongSurround>().textchange = true;
    }
    ...
    void Update() {
        if (gameObject.transform.position.y >= 4) {
            ...
            for (int i = 0; i < 4; i++) {
                transform.GetChild(i).GetComponent<WrongSurround>().canshoot = true;
                executeonce = true;
            }
        }
