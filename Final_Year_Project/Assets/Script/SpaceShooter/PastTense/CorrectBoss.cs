...
public class CorrectBoss : MonoBehaviour {
    ...
    void Start()
    {
        ...
        transform.GetChild(shoulddestroyship).GetComponentInChildren<Canvas>().GetComponentInChildren<Text>().text = JSONReader.instance.GetPastSimpleCorrect();
        transform.GetChild(shoulddestroyship).GetComponent<CorrectSurround>().textchange = true;
    }
    ...
    void Update()
    {
        if (gameObject.transform.position.y >= 4) {
            ...
            for (int i = 0;i < 4;i++) {
                transform.GetChild(i).GetComponent<CorrectSurround>().canshoot = true;
                executeonce = true;
            }
        }
