...
public class WrongBossPastParticiple : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        transform.GetChild(shoulddestroyship).GetComponentInChildren<Canvas>().GetComponentInChildren<Text>().text = JSONReader.instance.GetPastParticipleWrong();
        transform.GetChild(shoulddestroyship).GetComponent<WrongSurroundPastParticiple>().textchange = true;
    }
    ...
    void Update() {
        if (gameObject.transform.position.y >= 4) {
            ...
            for (int i = 0; i < 4; i++) {
                transform.GetChild(i).GetComponent<WrongSurroundPastParticiple>().canshoot = true;
                executeonce = true;
            }
        }
