...
public class GenerateRandomMcQuestionAnimalsTwo : MonoBehaviour
{
    ...
    public static GenerateRandomMcQuestionAnimalsTwo instance;
    ...
    async void ShowSavedDatabasePanel()
    {
        ...
        McQuestionQuiz currentResult = await DbHelper.GetEngVocabAnimalsTwoResultById(userId);
        ...
        if (currentResult == null)
        {
            ...
            bool success = await DbHelper.AddNewEngVocaAnimalsTwoTwoResult(userId, correctCount, totalQuestion, myTime, (canAttemptTimes-1));
        ...
        else
        {
            if (currentResult.attemptLeft > 0)
            {
                ...
                if (correctCount > currentResult.correctCount)
                {
                    ...
                    bool updateCorrectCountSuccess = await DbHelper.UpdateAnimalsTwoQuizCorrectCount(userId, correctCount);
                    bool updateTimesSuccess = await DbHelper.UpdateAnimalsTwoQuizTimes(userId, myTime);
                    bool updateAttempTimesSuccess = await DbHelper.UpdateAnimalsTwoQuizAttempt(userId, newAttempt);
