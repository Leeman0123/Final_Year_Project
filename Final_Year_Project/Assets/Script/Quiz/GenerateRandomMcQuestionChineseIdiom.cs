...
public class GenerateRandomMcQuestionChineseIdiom : MonoBehaviour
{
    ...
    public static GenerateRandomMcQuestionChineseIdiom instance;
    ...
    async void ShowSavedDatabasePanel()
    {
        ...
        McQuestionQuiz currentResult = await DbHelper.GetChineseP3IdiomResultById(userId);
        ...
        if (currentResult == null)
        {
            ...
            bool success = await DbHelper.AddNewChineseP3IdiomResult(userId, correctCount, totalQuestion, myTime, canAttemptTimes);
        ...
        else
        {
            if (currentResult.attemptLeft > 0)
            {
                ...
                if (correctCount > currentResult.correctCount)
                {
                    ...
                    bool updateCorrectCountSuccess = await DbHelper.UpdateChineseP3IdiomCorrectCount(userId, correctCount);
                    bool updateTimesSuccess = await DbHelper.UpdateChineseP3IdiomTimes(userId, myTime);
                    bool updateAttempTimesSuccess = await DbHelper.UpdateChineseP3IdiomAttempt(userId, newAttempt);
