...
public class GenerateRandomMcQuestionMathsSubtract : MonoBehaviour
{
    ...
    public static GenerateRandomMcQuestionMathsSubtract instance;
    ...
    async void ShowSavedDatabasePanel()
    {
        ...
        McQuestionQuiz currentResult = await DbHelper.GetMathsP1SubtractResultById(userId);
        ...
        if (currentResult == null)
        {
            ...
            bool success = await DbHelper.AddNewMathsP1SubtractResult(userId, correctCount, totalQuestion, myTime, canAttemptTimes);
        ...
        else
        {
            if (currentResult.attemptLeft > 0)
            {
                ...
                if (correctCount > currentResult.correctCount)
                {
                    ...
                    bool updateCorrectCountSuccess = await DbHelper.UpdateMathsP1SubtractCorrectCount(userId, correctCount);
                    bool updateTimesSuccess = await DbHelper.UpdateMathsP1SubtractTimes(userId, myTime);
                    bool updateAttempTimesSuccess = await DbHelper.UpdateMathsP1SubtractAttempt(userId, newAttempt);
