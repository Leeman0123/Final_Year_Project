...
public class GenerateRandomMcQuestionMathsMuDiv : MonoBehaviour
{
    ...
    public static GenerateRandomMcQuestionMathsMuDiv instance;
    ...
    async void ShowSavedDatabasePanel()
    {
        ...
        McQuestionQuiz currentResult = await DbHelper.GetMathsP2MuDivResultById(userId);
        ...
        if (currentResult == null)
        {
            ...
            bool success = await DbHelper.AddNewMathsP2MuDivResult(userId, correctCount, totalQuestion, myTime, canAttemptTimes);
        ...
        else
        {
            if (currentResult.attemptLeft > 0)
            {
                ...
                if (correctCount > currentResult.correctCount)
                {
                    ...
                    bool updateCorrectCountSuccess = await DbHelper.UpdateMathsP2MuDivCorrectCount(userId, correctCount);
                    bool updateTimesSuccess = await DbHelper.UpdateMathsP2MuDivTimes(userId, myTime);
                    bool updateAttempTimesSuccess = await DbHelper.UpdateMathsP2MuDivAttempt(userId, newAttempt);
