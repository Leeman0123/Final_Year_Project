...
public class GenerateRandomMcQuestionChineseFillIn : MonoBehaviour
{
    ...
    public static GenerateRandomMcQuestionChineseFillIn instance;
    ...
    async void ShowSavedDatabasePanel()
    {
        ...
        McQuestionQuiz currentResult = await DbHelper.GetChineseP1FillInResultById(userId);
        ...
        if (currentResult == null)
        {
            ...
            bool success = await DbHelper.AddNewChineseP1FillInResult(userId, correctCount, totalQuestion, myTime, canAttemptTimes);
        ...
        else
        {
            if (currentResult.attemptLeft > 0)
            {
                ...
                if (correctCount > currentResult.correctCount)
                {
                    ...
                    bool updateCorrectCountSuccess = await DbHelper.UpdateChineseP1FillInorrectCount(userId, correctCount);
                    bool updateTimesSuccess = await DbHelper.UpdateChineseP1FillInTimes(userId, myTime);
                    bool updateAttempTimesSuccess = await DbHelper.UpdateChineseP1FillInAttempt(userId, newAttempt);
