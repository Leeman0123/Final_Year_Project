...
public class GenerateRandomMcQuestionMathsDecimal : MonoBehaviour
{
    ...
    public static GenerateRandomMcQuestionMathsDecimal instance;
    ...
    async void ShowSavedDatabasePanel()
    {
        ...
        McQuestionQuiz currentResult = await DbHelper.GetMathsP3DecimalResultById(userId);
        ...
        if (currentResult == null)
        {
            ...
            bool success = await DbHelper.AddNewMathsP3DecimalResult(userId, correctCount, totalQuestion, myTime, canAttemptTimes);
        ...
        else
        {
            if (currentResult.attemptLeft > 0)
            {
                ...
                if (correctCount > currentResult.correctCount)
                {
                    ...
                    bool updateCorrectCountSuccess = await DbHelper.UpdateMathsP3DecimalCorrectCount(userId, correctCount);
                    bool updateTimesSuccess = await DbHelper.UpdateMathsP3DecimalTimes(userId, myTime);
                    bool updateAttempTimesSuccess = await DbHelper.UpdateMathsP3DecimalAttempt(userId, newAttempt);
                ...
                } else
                {
                    ...
                    attemptText.text = $"The quiz result will be saved, and you have {currentResultAttemptLeft} times chance";
                    bool updateAttempTimesSuccess = await DbHelper.UpdateMathsP3DecimalTimes(userId, newAttempt);
