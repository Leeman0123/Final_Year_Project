...
public class GenerateRandomMcQuestionMathsArith : MonoBehaviour
{
    ...
    public static GenerateRandomMcQuestionMathsArith instance;
    ...
    async void ShowSavedDatabasePanel()
    {
        ...
        McQuestionQuiz currentResult = await DbHelper.GetMathsP3ArithResultById(userId);
        ...
        if (currentResult == null)
        {
            ...
            bool success = await DbHelper.AddNewMathsP3ArithResult(userId, correctCount, totalQuestion, myTime, canAttemptTimes);
        ...
        else
        {
            if (currentResult.attemptLeft > 0)
            {
                ...
                if (correctCount > currentResult.correctCount)
                {
                    ...
                    bool updateCorrectCountSuccess = await DbHelper.UpdateMathsP3ArithCorrectCount(userId, correctCount);
                    bool updateTimesSuccess = await DbHelper.UpdateMathsP3ArithTimes(userId, myTime);
                    bool updateAttempTimesSuccess = await DbHelper.UpdateMathsP3ArithAttempt(userId, newAttempt);
