...
public class GenerateRandomMcQuestionPreposition : MonoBehaviour
{
    ...
    public static GenerateRandomMcQuestionPreposition instance;
    ...
    async void ShowSavedDatabasePanel()
    {
        ...
        McQuestionQuiz currentResult = await DbHelper.GetEngP3PrepositionResultById(userId);
        ...
        if (currentResult == null)
        {
            ...
            bool success = await DbHelper.AddNewEngP3PrepositionResult(userId, correctCount, totalQuestion, myTime, (canAttemptTimes-1));
        ...
        else
        {
            if (currentResult.attemptLeft > 0)
            {
                ...
                if (correctCount > currentResult.correctCount)
                {
                    ...
                    bool updateCorrectCountSuccess = await DbHelper.UpdateEngP3PrepositionCorrectCount(userId, correctCount);
                    bool updateTimesSuccess = await DbHelper.UpdateEngP3CompleteSentencesTimes(userId, myTime);
                    bool updateAttempTimesSuccess = await DbHelper.UpdateEngP3CompleteSentencesAttempt(userId, newAttempt);
                ...
                } else
                {
                    ...
                    attemptText.text = $"The quiz result will be saved, and you have {currentResultAttemptLeft} times chance";
                    bool updateAttempTimesSuccess = await DbHelper.UpdateEngP3PrepositionAttempt(userId, newAttempt);
