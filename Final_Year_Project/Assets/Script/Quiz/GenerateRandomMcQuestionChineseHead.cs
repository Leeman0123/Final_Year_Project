...
public class GenerateRandomMcQuestionChineseHead : MonoBehaviour
{
    ...
    public static GenerateRandomMcQuestionChineseHead instance;
    ...
    async void ShowSavedDatabasePanel()
    {
        ...
        McQuestionQuiz currentResult = await DbHelper.GetChineseP2HeadResultById(userId);
        ...
        if (currentResult == null)
        {
            ...
            bool success = await DbHelper.AddNewChineseP2HeadResult(userId, correctCount, totalQuestion, myTime, canAttemptTimes);
        ...
        else
        {
            if (currentResult.attemptLeft > 0)
            {
                ...
                if (correctCount > currentResult.correctCount)
                {
                    ...
                    bool updateCorrectCountSuccess = await DbHelper.UpdateChineseP2HeadCorrectCount(userId, correctCount);
                    bool updateTimesSuccess = await DbHelper.UpdateChineseP2HeadTimes(userId, myTime);
                    bool updateAttempTimesSuccess = await DbHelper.UpdateChineseP2HeadAttempt(userId, newAttempt);
