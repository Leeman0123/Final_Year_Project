...
public class GenerateRandomMcQuestionChineseUnit : MonoBehaviour
{
    ...
    public static GenerateRandomMcQuestionChineseUnit instance;
    ...
    async void ShowSavedDatabasePanel()
    {
        ...
        McQuestionQuiz currentResult = await DbHelper.GetChineseP1UnitResultById(userId);
        ...
        if (currentResult == null)
        {
            ...
            bool success = await DbHelper.AddNewChineseP1UnitResult(userId, correctCount, totalQuestion, myTime, canAttemptTimes);
        ...
        else
        {
            if (currentResult.attemptLeft > 0)
            {
                ...
                if (correctCount > currentResult.correctCount)
                {
                    ...
                    bool updateCorrectCountSuccess = await DbHelper.UpdateChineseP1UnitCorrectCount(userId, correctCount);
                    bool updateTimesSuccess = await DbHelper.UpdateChineseP1Unitimes(userId, myTime);
                    bool updateAttempTimesSuccess = await DbHelper.UpdateChineseP1UnitAttempt(userId, newAttempt);
