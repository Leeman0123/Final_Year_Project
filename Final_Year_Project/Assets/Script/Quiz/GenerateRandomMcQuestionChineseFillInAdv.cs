...
public class GenerateRandomMcQuestionChineseFillInAdv : MonoBehaviour
{
    ...
    public static GenerateRandomMcQuestionChineseFillInAdv instance;
    ...
    async void ShowSavedDatabasePanel()
    {
        ...
        McQuestionQuiz currentResult = await DbHelper.GetChineseP2FillInAdvResultById(userId);
        ...
        if (currentResult == null)
        {
            ...
            bool success = await DbHelper.AddNewChineseP2FillInAdvResult(userId, correctCount, totalQuestion, myTime, canAttemptTimes);
        ...
        else
        {
            if (currentResult.attemptLeft > 0)
            {
                ...
                if (correctCount > currentResult.correctCount)
                {
                    ...
                    bool updateCorrectCountSuccess = await DbHelper.UpdateChineseP2FillInAdvCorrectCount(userId, correctCount);
                    bool updateTimesSuccess = await DbHelper.UpdateChineseP2FillInAdvTimes(userId, myTime);
                    bool updateAttempTimesSuccess = await DbHelper.UpdateChineseP2FillInAdvAttempt(userId, newAttempt);
