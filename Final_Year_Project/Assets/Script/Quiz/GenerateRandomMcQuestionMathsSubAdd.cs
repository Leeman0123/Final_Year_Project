...
public class GenerateRandomMcQuestionMathsSubAdd : MonoBehaviour
{
    ...
    public static GenerateRandomMcQuestionMathsSubAdd instance;
    ...
    async void ShowSavedDatabasePanel()
    {
        ...
        McQuestionQuiz currentResult = await DbHelper.GetMathsP2SubAddResultById(userId);
        ...
        if (currentResult == null)
        {
            ...
            bool success = await DbHelper.AddNewMathsP2SubAddResult(userId, correctCount, totalQuestion, myTime, canAttemptTimes);
        ...
        else
        {
            if (currentResult.attemptLeft > 0)
            {
                ...
                if (correctCount > currentResult.correctCount)
                {
                    ...
                    bool updateCorrectCountSuccess = await DbHelper.UpdateMathsP2SubAddCorrectCount(userId, correctCount);
                    bool updateTimesSuccess = await DbHelper.UpdateMathsP2SubAddTimes(userId, myTime);
                    bool updateAttempTimesSuccess = await DbHelper.UpdateMathsP2SubAddAttempt(userId, newAttempt);
